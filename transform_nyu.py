...
class Resize(object):
    ...
    def __call__(self, img):
        ...
        if img.ndim == 3:
            ...
            #self.size = (int(img.shape[0]*self.size), int(img.shape[1]*self.size))
            h = int((float(img.size[0]) * self.size))
            w = int((float(img.size[1]) * self.size))
            img = Image.fromarray(img)
            
            return np.array((img.resize((w,h), resample=Image.Resampling.NEAREST)))
        
        elif img.ndim == 2:
            
            #return misc.imresize(img, self.size, self.interpolation, 'F')
            #self.size = np.int_(img.shape[0] * (self.size)), np.int_((img.shape[1] * (self.size)))
            h = int((float(img.shape[0]) * self.size))
            w = int((float(img.shape[1]) * self.size))
            #img = Image.fromarray(img)
            img  = np.array(Image.fromarray(((img).astype(np.uint8))).resize((h , w), resample=Image.Resampling.NEAREST))
            #img = convert_tensor(img).squeeze(0)
            #img = img.numpy()
            return img
            #return torch.fromnp.array((img.resize((h,w))))
        else:
